...
type questRows struct {
	Templates []content.SnapshotRow
	Rewards   []content.SnapshotRow
}

func mapQuestRows() (questRows, error) {
	questCatalog := quests.MustMVPQuestCatalog()
	templateRows, err := mapQuestTemplateRows(questCatalog)
	if err != nil {
		return questRows{}, err
	}
	rewardRows, err := mapQuestRewardRows(questCatalog)
	if err != nil {
		return questRows{}, err
	}
	return questRows{
...
func mapQuestTemplateRows(questCatalog quests.QuestCatalog) ([]content.SnapshotRow, error) {
	...
	for _, template := range templates {
		...
		row, err := snapshotRow(data.TemplateID.String(), data)
		if err != nil {
			return nil, err
		}
...
func mapQuestRewardRows(questCatalog quests.QuestCatalog) ([]content.SnapshotRow, error) {
	templates := questCatalog.Templates()
	sort.Slice(templates, func(i, j int) bool { return templates[i].TemplateID < templates[j].TemplateID })

	rows := make([]content.SnapshotRow, 0, len(templates))
	for _, template := range templates {
		data, err := questRewardRow(template)
		if err != nil {
			return nil, err
		}
		row, err := snapshotRow(data.RewardTableID.String(), data)
		if err != nil {
			return nil, err
		}
		rows = append(rows, row)
...
func questTemplateRow(template quests.QuestTemplate) (content.QuestTemplateRow, error) {
	...
	return content.QuestTemplateRow{
		...
		DifficultyRules: cloneQuestRawJSON(template.DifficultyRules),
		ObjectiveSchema: objectiveSchema,
		RewardRules:     cloneQuestRawJSON(template.RewardRules),
		ExpirationRules: cloneQuestRawJSON(template.ExpirationRules),
		Requirements:    append([]quests.QuestRequirement(nil), template.Requirements...),
		BoardWeight:     100,
	}, nil
}

func questRewardRow(template quests.QuestTemplate) (content.QuestRewardTableRow, error) {
	rewardTableID := catalog.DefinitionID(questRewardTablePrefix + template.TemplateID.String())
	source, err := catalog.NewQuestSource(rewardTableID.String(), quests.QuestCatalogVersion.String())
	if err != nil {
		return content.QuestRewardTableRow{}, err
	}
	payload := deterministicQuestRewardPayload(template)
...
func deterministicQuestRewardPayload(template quests.QuestTemplate) quests.RewardPayload {
	rank := minimumQuestTemplateRank(template.Requirements)
	...
	grants := []quests.RewardGrant{
		...
		},
		{
			...
			Role:   questRewardRoleForQuestType(template.Type),
			Amount: 15 + difficulty*15,
		},
	}
	if itemID := questRewardItemForQuestType(template.Type); !itemID.IsZero() {
...
func minimumQuestTemplateRank(requirements []quests.QuestRequirement) int {
	rank := 1
...
func questRewardRoleForQuestType(questType quests.QuestType) progression.RoleType {
	switch questType {
...
func questRewardItemForQuestType(questType quests.QuestType) foundation.ItemID {
	switch questType {
...
func cloneQuestRawJSON(raw json.RawMessage) json.RawMessage {
	if len(raw) == 0 {
		return nil
	}
