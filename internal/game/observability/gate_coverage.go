...
var phase12CommandSecurityProfiles = []commandSecurityProfile{
	...
	realtimeCommandSecurityProfile("combat.start_attack",
		satisfied(evidence("gameproject/internal/game/server", "TestCombatStartAttackRejectsClientAuthoredGameplayTruth", "combat.start_attack accepts only target intent and rejects spoofed combat facts")),
		satisfied(evidence("gameproject/internal/game/server", "TestCombatStartAttackQueuesStartedAndStateSnapshot", "combat.start_attack derives player/session identity from the authenticated command context")),
		notApplicable("combat.start_attack has no item/currency amount"),
		satisfied(evidence("gameproject/internal/game/server", "TestCombatStartAttackRejectsHiddenAndOutOfRangeBeforeStateMutation", "combat.start_attack validates visibility and range before mutating engagement state")),
		notApplicable("combat.start_attack mutates live combat engagement state, not item/currency value"),
		satisfied(evidence("gameproject/internal/game/server", "TestCombatStartAttackQueuesStartedAndStateSnapshot", "combat.start_attack queues a combat state snapshot after accepted state mutation")),
	),
	realtimeCommandSecurityProfile("combat.stop_attack",
		satisfied(evidence("gameproject/internal/game/server", "TestCombatStopAttackQueuesStoppedAndStateSnapshot", "combat.stop_attack accepts a stop intent without client-authored combat truth")),
		satisfied(evidence("gameproject/internal/game/server", "TestCombatStopAttackQueuesStoppedAndStateSnapshot", "combat.stop_attack resolves the active engagement from authenticated server state")),
		notApplicable("combat.stop_attack has no item/currency amount"),
		notApplicable("combat.stop_attack has no hidden target interaction; it clears the caller's active engagement"),
		notApplicable("combat.stop_attack mutates live combat engagement state, not item/currency value"),
		satisfied(evidence("gameproject/internal/game/server", "TestCombatStopAttackQueuesStoppedAndStateSnapshot", "combat.stop_attack queues a combat state snapshot after accepted state mutation")),
	),
	realtimeCommandSecurityProfile("combat.state",
		satisfied(evidence("gameproject/internal/game/server", "TestCombatStateReturnsCurrentPublicSnapshot", "combat.state returns the server-owned public engagement snapshot")),
		satisfied(evidence("gameproject/internal/game/server", "TestCombatStateReturnsCurrentPublicSnapshot", "combat.state resolves the caller's engagement from authenticated server state")),
		notApplicable("combat.state has no item/currency amount"),
		notApplicable("combat.state is a read operation over the caller's already-filtered combat engagement"),
		notApplicable("combat.state is read-only without item/currency mutation ledger"),
		notApplicable("combat.state is read-only without commit/broadcast semantics"),
	),
	realtimeCommandSecurityProfile("combat.select_ammo",
		satisfied(evidence("gameproject/internal/game/server", "TestCombatSelectAmmoRejectsClientAuthoredGameplayTruth", "combat.select_ammo accepts only family and item_id intent and rejects spoofed combat facts")),
		satisfied(evidence("gameproject/internal/game/server", "TestCombatSelectAmmoStoresServerOwnedSelectionAndQueuesState", "combat.select_ammo resolves account inventory from authenticated server state")),
		satisfied(evidence("gameproject/internal/game/server", "TestCombatSelectAmmoRequiresOwnedSelectableAmmo", "combat.select_ammo validates owned positive ammo quantity before selecting")),
		notApplicable("combat.select_ammo only changes the caller's active ammo setting; target visibility/range is enforced on attack ticks"),
		notApplicable("combat.select_ammo does not consume item value; laser ticks consume ammo through inventory ledger"),
		satisfied(evidence("gameproject/internal/game/server", "TestCombatSelectAmmoStoresServerOwnedSelectionAndQueuesState", "combat.select_ammo queues a combat state snapshot after accepted state mutation")),
	),
	...
	realtimeCommandSecurityProfile("inventory.move",
		satisfied(evidence("gameproject/internal/game/server", "TestInventoryMoveRejectsUnownedNegativeAndBlockedLocations", "inventory.move resolves the player from the session subject and accepts only item/location/quantity intent, rejecting trusted payload fields")),
		satisfied(
			evidence("gameproject/internal/game/economy", "TestMoveItemRejectsMissingRequiredInputs", "MoveItem rejects missing player, item definition, invalid locations, and same source/target before any mutation"),
			evidence("gameproject/internal/game/economy", "TestMoveItemRejectsZeroAndNegativeQuantity", "MoveItem rejects zero and negative quantities server-side"),
		),
		satisfied(evidence("gameproject/internal/game/server", "TestInventoryMoveDuplicateRequestDoesNotDoubleMoveStack", "inventory.move quantity is validated server-side and the move is idempotent per request reference")),
		notApplicable("inventory.move has no hidden target interaction; source ownership and location are validated server-side"),
		satisfied(evidence("gameproject/internal/game/economy", "TestMoveItemMovesStackableQuantityAndWritesLedgerEntries", "MoveItem writes item ledger entries for the moved quantity through the server-owned inventory service")),
		satisfied(evidence("gameproject/internal/game/server", "TestInventoryMoveDuplicateRequestDoesNotDoubleMoveStack", "inventory and cargo snapshots are queued to the session after the server-owned mutation commits")),
	),
	...
	realtimeCommandSecurityProfile("chat.send",
		satisfied(evidence("gameproject/internal/game/server", "TestChatSendRejectsSpoofedPlayerPayload", "chat.send rejects client-authored player identity fields")),
		satisfied(evidence("gameproject/internal/game/server", "TestChatSendQueuesLocalMapMembersOnly", "chat.send resolves sender and local-map membership from runtime server state")),
		notApplicable("chat.send carries no item/currency amount"),
		satisfied(evidence("gameproject/internal/game/server", "TestChatSendQueuesLocalMapMembersOnly", "chat.send routes local-map messages only to same-map members")),
		notApplicable("chat.send stores chat messages, not item/currency ledger mutations"),
		satisfied(evidence("gameproject/internal/game/server", "TestChatSendQueuesLocalMapMembersOnly", "chat.send queues realtime chat events after the chat service accepts the message")),
	),
	realtimeCommandSecurityProfile("party.invite",
		satisfied(evidence("gameproject/internal/game/server", "TestPartyInviteAcceptAddsMembership", "party.invite accepts invitee callsign intent and resolves player ids server-side")),
		satisfied(evidence("gameproject/internal/game/server", "TestPartyInviteAcceptAddsMembership", "party.invite creates/uses server-owned party membership for the authenticated inviter")),
		notApplicable("party.invite carries no item/currency amount"),
		notApplicable("party.invite has no hidden world target interaction"),
		notApplicable("party.invite mutates transient social membership, not item/currency ledger state"),
		satisfied(evidence("gameproject/internal/game/server", "TestPartyInviteAcceptAddsMembership", "party.invite queues party invite/update events through the post-command event drain")),
	),
	realtimeCommandSecurityProfile("party.accept",
		satisfied(evidence("gameproject/internal/game/server", "TestPartyInviteAcceptAddsMembership", "party.accept accepts only invite id intent and resolves invitee from the authenticated session")),
		satisfied(evidence("gameproject/internal/game/server", "TestPartyInviteAcceptAddsMembership", "party.accept adds exactly the authenticated invitee to server-owned party membership")),
		notApplicable("party.accept carries no item/currency amount"),
		notApplicable("party.accept has no hidden world target interaction"),
		notApplicable("party.accept mutates transient social membership, not item/currency ledger state"),
		satisfied(evidence("gameproject/internal/game/server", "TestPartyInviteAcceptAddsMembership", "party.accept queues party update events through the post-command event drain")),
	),
	realtimeCommandSecurityProfile("party.leave",
		satisfied(evidence("gameproject/internal/game/server", "TestPartyLeaveQueuesLeftEventForLeaver", "party.leave accepts an empty intent payload and resolves the leaver from the authenticated session")),
		satisfied(evidence("gameproject/internal/game/server", "TestPartyLeaveQueuesLeftEventForLeaver", "party.leave removes only the authenticated player from server-owned party membership")),
		notApplicable("party.leave carries no item/currency amount"),
		notApplicable("party.leave has no hidden world target interaction"),
		notApplicable("party.leave mutates transient social membership, not item/currency ledger state"),
		satisfied(evidence("gameproject/internal/game/server", "TestPartyLeaveQueuesLeftEventForLeaver", "party.leave queues party.left after the party service accepts the leave")),
	),
	realtimeCommandSecurityProfile("party.target.set",
		satisfied(evidence("gameproject/internal/game/server", "TestPartyTargetSetRequiresVisibleEntityAndQueuesUpdate", "party.target.set accepts only a target entity id and resolves the actor from the authenticated session")),
		satisfied(evidence("gameproject/internal/game/server", "TestPartyTargetSetRequiresVisibleEntityAndQueuesUpdate", "party.target.set mutates only the authenticated player's server-owned party")),
		notApplicable("party.target.set carries no item/currency amount"),
		satisfied(evidence("gameproject/internal/game/server", "TestPartyTargetSetRequiresVisibleEntityAndQueuesUpdate", "party.target.set requires the target entity to be visible to the setter before broadcasting")),
		notApplicable("party.target.set mutates transient social focus state, not item/currency ledger state"),
		satisfied(evidence("gameproject/internal/game/server", "TestPartyTargetSetRequiresVisibleEntityAndQueuesUpdate", "party.target.set queues party target updates after service acceptance")),
	),
	realtimeCommandSecurityProfile("clan.create",
		satisfied(evidence("gameproject/internal/game/server", "TestClanCreateJoinLeaveQueuesDurableSnapshots", "clan.create accepts only clan name/tag intent and resolves owner from the authenticated session")),
		satisfied(evidence("gameproject/internal/game/server", "TestClanCreateJoinLeaveQueuesDurableSnapshots", "clan.create writes owner membership for the authenticated player")),
		notApplicable("clan.create carries no item/currency amount"),
		notApplicable("clan.create has no hidden world target interaction"),
		notApplicable("clan.create mutates durable social membership, not item/currency ledger state"),
		satisfied(evidence("gameproject/internal/game/server", "TestClanCreateJoinLeaveQueuesDurableSnapshots", "clan.create queues clan snapshot events after service acceptance")),
	),
	realtimeCommandSecurityProfile("clan.join",
		satisfied(evidence("gameproject/internal/game/server", "TestClanCreateJoinLeaveQueuesDurableSnapshots", "clan.join accepts only clan tag intent and resolves joiner from the authenticated session")),
		satisfied(evidence("gameproject/internal/game/server", "TestClanCreateJoinLeaveQueuesDurableSnapshots", "clan.join writes membership for the authenticated player only")),
		notApplicable("clan.join carries no item/currency amount"),
		notApplicable("clan.join has no hidden world target interaction"),
		notApplicable("clan.join mutates durable social membership, not item/currency ledger state"),
		satisfied(evidence("gameproject/internal/game/server", "TestClanCreateJoinLeaveQueuesDurableSnapshots", "clan.join queues clan snapshot events through the post-command event drain")),
	),
	realtimeCommandSecurityProfile("clan.leave",
		satisfied(evidence("gameproject/internal/game/server", "TestClanCreateJoinLeaveQueuesDurableSnapshots", "clan.leave accepts an empty intent payload and resolves leaver from the authenticated session")),
		satisfied(evidence("gameproject/internal/game/server", "TestClanCreateJoinLeaveQueuesDurableSnapshots", "clan.leave removes only the authenticated player's durable membership")),
		notApplicable("clan.leave carries no item/currency amount"),
		notApplicable("clan.leave has no hidden world target interaction"),
		notApplicable("clan.leave mutates durable social membership, not item/currency ledger state"),
		satisfied(evidence("gameproject/internal/game/server", "TestClanCreateJoinLeaveQueuesDurableSnapshots", "clan.leave queues clan.left and remaining clan snapshots after service acceptance")),
	),
	...
	realtimeCommandSecurityProfile("admin.content.diff",
		satisfied(evidence("gameproject/internal/game/server", "TestAdminContentDiffRequiresAdminAndReportsChangedFields", "admin.content.diff rejects non-admin sessions and trusted payload fields")),
		satisfied(evidence("gameproject/internal/game/server", "TestAdminContentDiffRequiresAdminAndReportsChangedFields", "admin.content.diff resolves admin role from the authenticated server session and scrubs secret values")),
		notApplicable("admin.content.diff has no client-authored amount"),
		notApplicable("admin.content.diff has no hidden world target interaction"),
		notApplicable("admin.content.diff is a read-only CMS comparison without item/currency mutation ledger"),
		notApplicable("admin.content.diff is read-only without commit/broadcast semantics"),
	),
