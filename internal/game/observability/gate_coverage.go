...
var phase12CommandSecurityProfiles = []commandSecurityProfile{
	...
	realtimeCommandSecurityProfile("auction.claim_grant",
		satisfied(evidence("gameproject/internal/game/server", "TestPhase08MarketAuctionPremiumUseServerEconomyState", "auction.claim_grant returns only grants for the authenticated player")),
		notApplicable("auction.claim_grant uses the server-resolved session subject instead of a client-owned player id"),
		notApplicable("auction.claim_grant has no client-authored amount"),
		notApplicable("auction.claim_grant has no hidden world target interaction"),
		notApplicable("auction.claim_grant exposes skeleton grant snapshots; concrete item/unlock grant adapters remain future work"),
		notApplicable("auction.claim_grant is read-only in the current skeleton grant slice"),
	),
