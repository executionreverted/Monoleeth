...
- [x] `[P:wave1/lane-B]` Add bounded per-session event ring keyed by `seq`; replay missed events on reconnect before latest snapshot.
...
- [x] Reconnect with last `seq` replays missed events in order.
...
- [x] Reconnect recovers missed bounded events deterministically.
