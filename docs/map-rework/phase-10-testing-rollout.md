...
- Authenticated `route.create`, `route.update`, `route.enable`, and
  ...
  routes. Focused server coverage proves create derives owner, route id, and
  ...
  mutation, and keeps stored-route/config errors internal. Browser route
  create/update/control/settle proof and durable DB/outbox/window idempotency
  remain open.
