...
| Rollout/migration controls | Partial | `docs/running-local-game.md`, `docs/test-server-operations.md`, future persistence migration docs | Proposed flag only: `GAME_FEATURE_BOUNDED_MULTI_MAP`; future migration/backfill jobs | Private test-server runbook now defines artifact, seed, reset, port/origin, and rollback policy for the current process-local playtest server | No durable flag/backfill/quarantine/rollback tests yet | N/A | Current in-memory/dev runtime already routes through bounded multi-map behavior. Private playtest ops are documented; production rollout still needs a durable flag/backfill plan if DB persistence is introduced. |
...
- Focused scanner rarity/hidden-scan-data regression exists in
  ...
  combat/drop/pickup path. The focused built-client
  `e2e:phase10-scan-no-signal` proof now covers browser `no_signal` responses
  for both a dev-seeded planet-candidate miss and a stealthed player outside
  radar range without exposing hidden player, scan, planet-candidate, or map
  internals. Full per-map drop matrix variants remain missing.
...
Private test-server operations for the current process-local runtime are now
documented in `docs/test-server-operations.md`. That runbook covers the
supported one-process Go server plus built client shape, playtest seed policy,
reset expectations, port/origin config, artifact staging, and rollback steps.
It does not replace the future durable DB migration/backfill plan.
...
Run the same proof against the built client served by the Go server:
...
Run the focused built-client Phase10 scanner no-signal browser proof
explicitly:
...
That proof starts the Go server with `GAME_DEV_MODE=1` and
`GAME_E2E_SCAN_NO_PLANET_SEED=1`, registers three real browser players, proves
a planet-candidate `no_signal` scan without materializing known planets, then
moves one stealthed target outside starter radar but inside scan radius, sends a
second real `scan.pulse`, and expects client-safe `no_signal` response/state
without hidden player, planet-candidate, scan, map, session, storage,
WebSocket, or process-log leaks.
...
built-client PvP/death/repair loop, Border Skirmish enemy aggro/leash canary,
destination/PvP scanner, claim, plus Border Skirmish drop canary, and scanner
