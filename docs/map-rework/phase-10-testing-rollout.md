...
- No full per-map scanner/claim/drop seed matrix. Focused server-only proofs now
  cover seeded destination map `map_1_2` / public `1-2` for claim success and
  `scan.pulse` active-map scan scope/read-model isolation, but scanner/drop
  matrix coverage and browser claim flow remain open.
...
- Focused scanner rarity/hidden-scan-data regression exists in
  ...
  proof for public `1-2`, and Phase09 now covers one browser scan pulse with
