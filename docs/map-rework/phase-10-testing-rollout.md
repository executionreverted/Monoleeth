...
| Planet claim/production/routes | Partial | `internal/game/server/server_planet_claim_test.go`, `internal/game/server/server_discovery_planets_test.go`, `internal/game/server/server_route_control_test.go`, `internal/game/production/route_test.go`, `internal/game/production/route_mutation_test.go`, `client/tests/e2e/phase10-planet-claim-flow.mjs`, `docs/map-rework/phase-07-planet-claim-production-routes.md` | `discovery.claim_planet`, `planet.claimed`, `discovery.known_planets`, `discovery.planet_detail`, `planet.production_summary`, `planet.storage_summary`, `route.create`, `route.enable`, `route.disable`, `route.list`, `route.snapshot`, future `route.update/settle` | `server_planet_claim`: `TestClaimPlanetSucceedsForKnownNearbyPlanetAndEmitsSafeOwnerEvents`, `TestClaimPlanetDuplicateRetryDoesNotConsumeSecondXCore`, `TestClaimPlanetSucceedsOnSeededDestinationMap` for public `1-2`, `TestClaimPlanetSucceedsOnSeededPVPMap` for public `1-3`; focused Phase10 browser claim proof registers a real user, scans, opens real planet detail, sends the real HUD `discovery.claim_planet` click path, consumes one E2E-seeded X Core through Inventory, uses E2E-seeded Progression rank for claim eligibility, receives `planet.claimed`, reconciles production/inventory, and checks leak canaries; production route: `TestCreateRouteDistanceAndRiskCalculation`, `TestCreateRouteStoresDetachedEnabledRoute`, `TestAutomationRouteValidateRequiresMapIdentity`; server route gateway: `TestRouteCreateCreatesOwnedPlanetRouteThroughGateway` proves `route.create` derives owner, route id, endpoint map ids, and owner-scoped safe route events server-side; `TestRouteControlDisableThenEnableThroughGatewayQueuesSafeOwnerEvents` proves `route.disable` and `route.enable` toggle the owned route through safe response/list/snapshot/event payloads; `TestRouteDisableSettlesStorageAndQueuesSafeProductionStorageSnapshots` proves elapsed disable settlement reconciles active-map production/storage snapshots | `server_planet_claim`: trusted/unknown payload, missing X Core, cross-map, out-of-range, low-rank rejection tests; protocol/reducer/UI tests prove the browser claim intent only sends `planet_id`, clears pending state, and handles `planet.claimed` without an unhandled-event log; production route: ownership, destination, non-routeable, unsupported destination, non-positive rate, requirement, policy map identity tests; server route gateway: `TestRouteCreateRejectsSpoofedServerOwnedFieldsBeforeMutation` rejects forged owner/map/energy/risk fields before mutation, `TestRouteControlRejectsSpoofedServerOwnedFieldsBeforeMutation` rejects forged owner/map/enabled/settlement/source/destination/storage/risk fields before mutation, `TestRouteControlRejectsWrongOwnerWithoutMutationOrEvents` rejects wrong-owner controls without mutation/events, `TestRouteControlMapsStoredRouteConfigErrorsToInternal` keeps stored-route/config errors internal, and `TestRouteMutationOperationsRemainUnregisteredGatewayQuarantine` keeps authenticated `route.update` and `route.settle` requests quarantined as invalid payload; audit gap: route update/settle handlers/contracts and browser route create/control proof still open | Focused claim proof via `npm --cache /tmp/gameproject-npm-cache --prefix client run e2e:phase10-planet-claim`; no route browser artifact | Destination-map server claim success is covered for seeded `map_1_2` / public `1-2` and PvP `map_1_3` / public `1-3`, including safe payload/event and one-X-Core consumption assertions. Browser claim proof now covers the starter-map real-client claim path with an E2E-only Inventory X Core plus Progression rank seed. Browser drop flow, the full claim/drop matrix, browser route create/control proof, route update/settle mutations, and durable DB/outbox/backfill remain open. |
...
- Focused scanner rarity/hidden-scan-data regression exists in
  ...
  DOM/state/storage/cookie leak checks, and Phase10 now has a focused browser
  claim proof. Drop matrix coverage and broader browser scan variants remain
  missing.
- Authenticated `route.create`, `route.enable`, and `route.disable` now have
  server gateway slices for owned planet-to-planet MVP routes. Focused server
  coverage proves create derives owner, route id, and endpoint map ids
  server-side, while enable/disable derive owner from the authenticated context,
  toggle the stored route through safe response/list/snapshot/event payloads,
  reconcile active-map production/storage snapshots when disable settlement
  touches storage, reject wrong-owner attempts without mutation/events, keep
  ...
  mutation. `route.update` and `route.settle` remain quarantined as invalid
  payload until their handlers land.
