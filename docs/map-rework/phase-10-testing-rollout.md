...
- PvP-enabled map seed now exists as public `1-3` / Border Skirmish, reachable
  ...
  protection and client-safe position/protection events. Real browser PvP
  death/cargo/checkpoint proof and real browser safe-zone PvP click proof remain
  missing.
...
- Browser PvP death/cargo/checkpoint tests are missing; current coverage is
  limited to the death-domain `ProcessDeath` handoff shape and the seeded-map
  runtime command path, including server-side repair checkpoint snap and
  respawn-protection proof.
