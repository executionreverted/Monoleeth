...
- Phase09 now covers inbound/outbound WebSocket text frames,
  ...
  text scan now covers fake/default fixture labels/ids and server-only content
  ids. Production logs beyond this harness, admin/debug responses outside this
  rejection path, non-Phase09 WebSocket paths, source-map scans,
  published-artifact scans, and screenshot paths outside the Phase09 smoke are
  still missing.
...
rejection path, source-map/published artifact bundle scans, non-Phase09
