...
- Focused scanner rarity/hidden-scan-data regression exists in
  ...
  missing.
- Authenticated `route.create` now has a server gateway slice for owned
  planet-to-planet MVP routes. Focused server coverage proves it derives owner,
  route id, and endpoint map ids server-side, emits owner-scoped safe
  `route.updated`/`route.snapshot`/`route.list` events, reconciles through
  `route.list` and `route.snapshot`, and rejects forged owner/map/energy/risk
  fields before mutation. `route.update/enable/disable/settle` remain
  quarantined as invalid payload until their handlers land.
