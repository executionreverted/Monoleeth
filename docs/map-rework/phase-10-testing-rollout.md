...
| Bounded scanner/planets | Partial | `internal/game/discovery/candidate.go`, `internal/game/discovery/scanner.go`, `internal/game/server/server_discovery_planets_test.go`, `docs/map-rework/phase-06-bounded-scanner-planets.md` | `scan.pulse`, `scan.pulse_started`, `scan.pulse_resolved`, `scan.planet_discovered`, `discovery.known_planets`, `discovery.planet_detail` | discovery candidate/scanner: `TestPlanetCandidateGenerationLowDensityBlocksOtherwiseEligibleCell`, `TestResolveScanPulseDiscoversPlanetWritesIntelEventAndXPOnce`, `TestResolveScanPulseMaterializationAndIntelAreSeededMapScoped` covers domain scanner materialization/intel across seeded public maps `1-1`, `1-2`, and `1-3`, `server_discovery_planets`; `TestScanPulseUsesActiveSeededMapScope` proves authenticated runtime sessions active on `map_1_2` and `map_1_3` scan through server-owned active map scope and keep response/event/read-model memory on public `1-2` and `1-3`; client e2e phase09 covers one authenticated browser scan pulse in the full loop; `e2e:phase10-scan-no-signal` proves a built-client browser `no_signal` scan against a stealthed player outside radar but inside scan radius | discovery candidate/scanner/server: `TestPlanetCandidateGenerationFiltersByBiomeAndSpawnBudget`, `TestStartScanPulseCooldownBlocksSpamWithoutMutation`, `TestStartScanPulseEnergyUnavailableFailsBeforeCooldownAndMutation`, `TestResolveScanPulseRadarTooLowReturnsGenericNoSignal`, `TestResolveScanPulseResultJSONOmitsHiddenTruth`, `TestPhase07DiscoveryProductionRouteQueriesUseServerState` and `TestScanPulseUsesActiveSeededMapScope` hidden-scan-data response/event/world snapshot assertions; audit gaps: no full per-map scanner/claim/drop seed matrix or browser planet-candidate no-signal matrix | Phase09 browser proof covers one scan pulse, and Phase10 now has a focused browser hidden-player no-signal canary; this is not the full per-map scanner matrix | Keep dev/test scanner seed deterministic and separate from production rarity tuning. |
...
- Focused scanner rarity/hidden-scan-data regression exists in
  ...
  for a stealthed player outside radar range without exposing hidden player,
  scan, or map internals. Broader planet-candidate no-signal and full per-map
  drop matrix variants remain missing.
...
That proof registers two real browser players, moves one stealthed target
outside starter radar but inside scan radius, sends a real `scan.pulse`, and
expects a client-safe `no_signal` response/state without hidden player, scan,
map, session, storage, WebSocket, or process-log leaks.
