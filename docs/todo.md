...
- [x] P13/P15: prove load-envelope bounds for AOI payload size and aggro
  ...
  one NPC tick, one spatial candidate check). Runtime tick-stability proof now
  covers 128 active sessions inside the targeted-race wall-clock budget. Full
  1500-session runtime AOI projection proof remains open in
  `docs/road-to-v1/15-world-performance-aoi-optimization.md`.
- [x] P13: complete release evidence suite. OTel command/tick spans,
  production/route deterministic summary smokes, runtime AOI tick-stability
  smoke, command+tick+economy mutation race target, and final release-gate
  coverage evidence now pass. Source:
...
- [x] P13: prove release-gate coverage fails closed for missing evidence. The
  ...
  one required module/check evidence item fails the report. Source:
  `docs/road-to-v1/13-observability-simulation-release.md`.
- [x] P13: prove one economy simulation balance scenario. The
  ...
  evidence references that proof. Source:
  `docs/road-to-v1/13-observability-simulation-release.md`.
- [x] P13: prove combat/loot simulation determinism. The combat/loot simulation
  ...
  Source: `docs/road-to-v1/13-observability-simulation-release.md`.
...
- [x] P15: replace NPC aggro full-player scan with a worker-owned player
  ...
  move/settle/speed/tick movement paths, and focused worker tests prove target
  parity plus no full player scan. Source:
  `docs/road-to-v1/15-world-performance-aoi-optimization.md`.
- [x] P10: finish social moderation redaction/logging policy and party/clan
  contribution event semantics. Runtime now redacts chat PII/secrets before
  storage/fanout, logs only keyed HMAC fingerprints plus safe moderation
  metadata, and emits party/clan contribution read models from server-owned
  NPC-kill contribution maps with opaque occurrence ids. Source:
...
- [ ] Finish the DB/CMS content pipeline after the static gameplay content
  ...
  projected material item aliases, fully Kalaazu-derived buyable shop rows, and
  Kalaazu-resource loot tables, with source/imported/unsupported counts
  recorded by the import report. Legacy static catalog rows still bridge
  systems Kalaazu does not define yet: XP, scanner/cargo/special item support,
  recipes, production buildings, quests, scanner config, starter config, route
  policy, production rules, and combat rules. Kalaazu speed-generator items now
  map into module speed stats. Next
...
- [ ] Add a deterministic per-map scanner/claim/drop seed matrix. Focused
  ...
  for a dev-seeded planet-candidate miss and a stealthed player outside radar
  but inside scan radius, with DOM/state/storage/WebSocket/process-log leak
  canaries. Remaining work is broader browser claim/drop and full per-map
  matrix variants.
  Source:
  `docs/map-rework/phase-10-testing-rollout.md`.
...
- [ ] Complete remaining Phase10 leak canaries over production logs beyond the
  ...
  build-only gate for hosted CI/deploy jobs. The GitHub Actions workflow is
  active at `.github/workflows/playtest-artifact-gate.yml` and uses the template
  in `docs/ci/playtest-artifact-gate-github-actions.yml`; it runs the same gate
  on pull requests, pushes to `master`, and manual dispatch. The first hosted
  run URL/status still needs to be recorded.
  `client/tests/bundle-scan-extra-root-regression.mjs`
  ...
  beyond the playtest asset screenshot, and external deploy-job integration
  remain open.
  Source:
  `docs/map-rework/phase-10-testing-rollout.md`.
...
- [x] Add a single-command built-client vertical-slice verification gate.
  ...
  loop, destination/PvP scanner plus Border Skirmish drop canary, and scanner
  no-signal canary. The default gate now reuses the scanned `client/dist`
...
- [ ] Replace remaining explicit Kalaazu default-seed legacy bridge rows.
  ...
  default snapshot row explicit and tested, but scanner/cargo/special
  item/module compatibility rows, crafting, production, quests, scanner config,
  starter config, route policy, production rules, and combat rules still need
  domain-owned Kalaazu/default replacements. Ship rows, shop products, loot
  tables, starter laser/shield module rows, their item rows, and material item
  aliases are now produced by the Kalaazu default seed builder. Source:
