...
- [ ] Add pending/complete or compensation handling around Phase 08 coordinate
  ...
  authenticated `intel.coordinate_item.create` command. Phase07CV exposes
...
- [ ] Finish authenticated planet ownership/building mutation contracts.
  ...
  remain open. Phase07CK adds browser HUD controls and command-builder wiring
  for owned-planet building build/upgrade intents; the client sends only
  planet/building/slot/target-level intent fields and reconciles from server
  production summary events. Phase07Z adds a store-owned begin/complete claim
...
- [ ] Finish durable authenticated automation route persistence and rollout
  ...
  public map-key payloads. Phase07CS makes browser `route.list` responses,
  ...
  Phase07BR, Phase07BS, Phase07BT, Phase07BV, Phase07CD, Phase07CI,
  Phase07CJ, and Phase07CS.
...
- [ ] Add station/storage destination settlement adapters for Phase 09
  ...
  publishers. Phase07CQ stores the last browser-safe route settlement result in
  client state and renders source-empty, destination-full, no-transfer,
  settled, and loss-applied outcomes in route rows without leaking masked
  storage/station aggregate ids; DB-backed storage/station endpoint rows and row locks remain open.
