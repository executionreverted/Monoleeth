...
server-owned game rules. This phase is the final audit, screenshot,
browser/e2e, and documentation update pass.

Current state: this gate is not executable yet. The monolithic browser smoke
suite was deleted, `npm run check:task-001` does not exist, and the artifact
verifier does not exist. Rebuild the gate through
[`browser-e2e-rebuild-plan.md`](./browser-e2e-rebuild-plan.md) before closing
any release checklist items.
...
- Abuse/security evidence covers rate limits, idempotency, ledger references,
  ...
  gameplay truth.
- Browser/e2e revoked-session/auth-expiry flow revokes or expires a live
  session, sends a command, observes `auth_expired`, terminal socket close,
  pending command cleanup, and no stale gameplay state.
...
- Browser/e2e no longer expects `fog.active === true` or
  `radar_range === 420` as a passing condition for Task 001.
...
- Shop browser/e2e asserts game category rail, `shop.catalog` or equivalent
  metadata, selected detail, no raw ids, and no `server_recalculates`
  copy/state expectation in normal player UI.
...
- Update future browser/e2e artifact paths from old `ui-implementation`,
...
- Update unimplemented-control browser/e2e so disabled primary placeholders
  cannot pass as acceptable player UI. Impossible actions should be hidden,
  quiet, or named blockers in the owning phase.
- Include client browser/e2e gates for `?` tutorial help, content-sized windows,
...
- Replace old shop browser/e2e truth: no primary
  `Market/Sell/Auction/Premium` tab expectation, no `raw_ore` purchase fixture,
  and no `server_recalculates` visible/state expectation in normal player UI.
...
- Add blocked-contract gate coverage for `inventory.move` and
  ...
  from the protocol.
- Add concrete Phase 10 browser/e2e gates for valid `1`, valid `6`, `Tab`,
  WASD decision, radar contact click, and modal/window click isolation after
  timer expiry.
...
- The final screenshot gate can false-pass if it only runs `find`. Browser/e2e
  must write final artifacts under `output/screenshots/task-001/`, and a script
  must assert the exact required filenames, non-empty file size, and mtime newer
  than the run start.
- Shop browser/e2e must fail on old wrong truth: primary
...
- Planet browser/e2e must fail when unavailable `Claim`, `Build`, `Upgrade`,
  `Route`, or `Auto` primary controls are visible disabled placeholders.
  Acceptable states are hidden, quiet game copy, or a named blocker in Phase 08.
- Browser/e2e must include revoked-session/auth-expiry: revoke or expire a
  live session, send a command, observe `auth_expired`, terminal socket close,
  pending cleanup, and cleared gameplay state.
- Browser/e2e must include scanner witness with multiple contexts:
  hidden target absent, scanner reveal returns safe
  `player_revealed`/`scan_revealed`, unrelated viewer cannot see, and expiry
  removes visibility.
- Quest browser/e2e must click claim and reroll happy paths when fixtures expose
  them; otherwise the final gate must list the exact fixture blocker.
- Phase 10 browser/e2e must include `Tab` target cycle, WASD
  implemented-or-absent check, radar/minimap contact behavior, and delayed
  modal/window focus isolation after the old suppression timeout would have
  expired.
...
- Add a current-run manifest at `output/screenshots/task-001/run-manifest.json`
  with run start time, git SHA/dirty state, command, auth mode, server URL,
  viewport list, screenshot hashes, and whether the run used real server state.
- Add a future `client` task such as `npm run check:task-001` after the rebuilt
  harness and artifact verifier exist. It should run the normal client check
  plus artifact verification; Phase 11 should use that as the final client gate
  instead of relying on a manual artifact verifier command.
...
- Screenshot/browser-e2e artifacts must prove real-server provenance. Demo/fixture
  paths such as `?demo=1` or test-only fixtures may be used only when the
  manifest names them as non-release artifacts.
- Add three-session browser/e2e coverage: non-hidden player visible baseline, target
...
- Add negative real WebSocket browser/e2e coverage for `debug_snapshot` and
  `debug_spawn_npc`: non-dev server rejects them, and normal real-client
  command logs contain no debug operations.
...
Future full verification after the per-flow browser/e2e harness and artifact
verifier exist:
...
deleted monolithic suite. The `check:task-001` command and artifact verifier
above are future Phase 11 requirements, not current commands.
...
- [ ] `docs/plans/ui-implementation/*` is updated if an active phase status
      changed.
- [ ] Future browser/e2e harness uses authenticated real server state.
- [ ] Future screenshot/e2e run writes `run-manifest.json` with current-run
      provenance, hashes, git state, server URL, viewport list, and auth mode.
- [ ] Screenshots are current and named.
- [ ] Future browser/e2e harness captures Task 001 screenshots under
      `output/screenshots/task-001/`.
- [ ] Required screenshot filenames exist in the flat Task 001 directory,
      are non-empty, and come from the current browser/e2e run.
- [ ] Screenshot artifact policy is script-enforced with exact filenames,
      non-empty sizes, and current-run mtimes.
- [ ] Future `npm run check:task-001` runs client check plus artifact
      verification.
...
- [ ] Browser/e2e asserts no generic `Inspect`, `server-owned`, `server policy`,
      `server recalculates`, `No server-owned routes`, or similar internal copy
      in normal player UI text, `title`, and `aria-label`.
- [ ] Forbidden-copy browser/e2e scans visible normal-player text plus `title`,
...
- [ ] Phase 10 browser/e2e covers valid `1`, valid `6`, `Tab`, WASD decision,
      radar contact click, and modal/window click isolation after timer expiry.
- [ ] Three-session stealth/witness browser/e2e covers visible baseline, stealth
      disappearance, scanner reveal, unrelated viewer absence, expiry removal,
      and witnessed-target interaction allow/deny.
- [ ] Negative real WebSocket browser/e2e proves debug operations are forbidden
      in non-dev mode and never sent by normal real-client flows.
- [ ] Shop browser/e2e fails on old `Market/Sell/Auction/Premium`, `raw_ore`,
      and `server_recalculates` expectations.
- [ ] Planet browser/e2e fails on visible disabled unavailable primary
      controls.
