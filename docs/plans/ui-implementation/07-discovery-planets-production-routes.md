...
- [x] Browser coordinate item use pending state clears from authoritative
      inventory snapshots after the server consumes the scroll, while partial
      inventory payloads keep the lock.
...
- [x] Runtime route create/update/enable/disable writes durable route-row
      ...
      advancement.
- [x] Authenticated duplicate `route.enable`/`route.disable` request IDs replay
      the original safe gateway response when the retry route id changes,
      without mutating the second route, energy reservation, or event batch.
