...
export class ClientApp extends ClientAppHandlers {
  ...
  async start(): Promise<void> {
    ...
    this.hud = new HUD(hudHost, {
      ...
      onChatSend: (kind, content) => this.sendChatMessage(kind, content),
      onPartyInvite: (callsign) => this.sendPartyInvite(callsign),
      onPartyAccept: (inviteID) => this.sendPartyAccept(inviteID),
      onPartyLeave: () => this.sendPartyLeave(),
      onPartyTargetSet: (targetID) => this.sendPartyTargetSet(targetID),
      onClanCreate: (name, tag) => this.sendClanCreate(name, tag),
      onClanJoin: (tag) => this.sendClanJoin(tag),
      onClanLeave: () => this.sendClanLeave(),
