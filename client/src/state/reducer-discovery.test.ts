...
describe('reduceClientState', () => {
  ...
  test('route updated event clears matching pending route mutations and upserts safe route state', () => {
    const initialRoute = routeFixture({ enabled: false, amount_per_hour: 10 });
    const seeded = routeSeedState(initialRoute);
    seeded.pendingCommands = {
      ...
      'route-enable-1': {
        requestID: 'route-enable-1',
        op: OPERATIONS.routeEnable,
        queuedAt: 1,
        payload: { route_id: 'route-1' },
      },
      'route-disable-1': {
        requestID: 'route-disable-1',
        op: OPERATIONS.routeDisable,
        queuedAt: 1,
        payload: { route_id: 'route-other' },
      },
      'claim-1': {
        requestID: 'claim-1',
        op: OPERATIONS.discoveryClaimPlanet,
    ...
    const next = reduceClientState(seeded, {
      ...
      envelope: event(
        ...
        {
          route: routeFixture({ enabled: true, amount_per_hour: 25, updated_at: 1400 }),
        },
        4,
      ),
    });
  ...
  test('route settled event clears pending settlement and reconciles route list plus storage', () => {
    const initialRoute = routeFixture({ amount_per_hour: 40 });
    const seeded = routeSeedState(initialRoute);
    seeded.pendingCommands = {
      'route-settle-1': {
      ...
      'route-settle-other': {
        requestID: 'route-settle-other',
        op: OPERATIONS.routeSettle,
        queuedAt: 1,
        payload: { route_id: 'route-other' },
      },
      'route-settle-reconcile': {
        requestID: 'route-settle-reconcile',
        op: OPERATIONS.routeSettle,
        queuedAt: 1,
        payload: {},
      },
